  ...
  public static Argument<String> CurrencyArgument() {
    return new CustomArgument<>(new StringArgument("currency"), info -> {
      ...
      if (!StorageManager.getActiveStorageType().getCurrencies().contains(currency))
        throw CustomArgument.CustomArgumentException.fromString(Messages.getParsedMessage(MessageType.CURRENCY_NOT_FOUND,
          "currency", currency));
      return currency;
  ...
  public static Argument<OfflinePlayer> TargetArgument(boolean mustBeOnline) {
    return new CustomArgument<>(new OfflinePlayerArgument("target"), info -> {
      ...
      if (!QualityEconomyAPI.hasAccount(target.getUniqueId()))
        throw CustomArgument.CustomArgumentException.fromString(Messages.getParsedMessage(MessageType.PLAYER_NOT_FOUND,
          "player", target.getName()));
      if (mustBeOnline && !target.isOnline())
        throw CustomArgument.CustomArgumentException.fromString(Messages.getParsedMessage(MessageType.PLAYER_NOT_ONLINE,
          "player", target.getName()));
      return target;
  ...
  public static Argument<Double> AmountArgument() {
    return new CustomArgument<>(new StringArgument("amount"), info -> {
      ...
      try {
        ...
        throw CustomArgument.CustomArgumentException.fromString(Messages.getParsedMessage(MessageType.INVALID_NUMBER,
          "amount", rawAmount));
      }
